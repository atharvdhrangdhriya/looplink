...
console.log('profileData',profileData);
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (userdata.social_media) {
        const socialMediaData = JSON.parse(userdata.social_media);
  ...
  const cleanSocialMediaString = (socialMediaString) => {
    try {
      // Parse the string into an object
      const socialMediaObject = JSON.stringify(socialMediaString);
      
      // Remove unwanted keys
      //const { 0: _, 1: __, 2: ___, 3: ____, 4: _____, 5: ______, ...cleanedObject } = socialMediaObject;
      
      // Convert back to a string if needed
      return socialMediaObject;
    } catch (error) {
      console.error("Error parsing social media string:", error);
      return socialMediaString; // return the original string if parsing fails
    }
  };
  
  const fetchSocialMediaData = async () => {
    // Assume you fetch this data from an API or another source
    const response = await fetch('your_api_endpoint_here');
    const data = await response.json();
    
    // Assume data.social_media contains your social media string
    const cleanedSocialMediaString = cleanSocialMediaString(data.social_media);
    
    // Update state with cleaned data
    setSociallinks(JSON.parse(cleanedSocialMediaString));
  };
  ...
  const EditProfile = async (e) => {
   ...
   if (image == "undefined") {
     console.log('image is undefined or null');
      ...
      console.log('profileDataadsas',profileData);
      ...
      const response = await fetch('http://localhost:5000/upedateProfile', {
        ...
        //body: JSON.parse(profileData), // Corrected to stringify the profile data
        body: profileData
      });
      ...
      if (result.message) {
        let Session = JSON.stringify(sessionStorage.getItem('user'));
        Session.Fullname = profileData.Fullname;
        Session.Profilename = profileData.Profilename;
        Session.Bio = profileData.Bio;
